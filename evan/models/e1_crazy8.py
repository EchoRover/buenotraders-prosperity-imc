"""
e1_crazy8 — claude2 agent
=========================
2,527 baseline (crazy7). Push for 3,000.

EMERALDS (1,050 → 1,200+ target):
  More aggressive CLEAR cycling. crazy1 uses pos>30→15.
  Each cycle: lose ~15 on clear, gain ~36 on next fill = +21 net.
  If we cycle at pos>20→5: faster cycles → more fills.
  29 fills currently. Target: 35+ fills = 1,260+.

TOMATOES (1,477 → 1,800+ target):
  Claude1 proved we're directional (79.5% favorable fills, +1,662 directional).
  v10's soft limit at pos>10 cuts us SHORT — starts managing inventory
  when we should be RIDING the position.

  Changes from v10 SUBMITTED:
  1. T_SOFT_LIMIT: 10 → 20 (let positions ride longer)
  2. Hard limit: 50 → 70 (more capacity for directional bets)
  3. LIMITS["TOMATOES"]: 50 → 70 (match hard limit)

  Why this could work:
  - With 79.5% favorable fills, larger positions = more profit per move
  - Soft limit at 10 was too conservative — flattens winning positions too early
  - Mr Nobody probably lets positions run bigger ("simple MM + unwinding")
...
# v10 SUBMITTED TOMATOES constants (only soft/hard limit changed)
T_ADVERSE_VOL = 15
...
T_DISREGARD = 1
T_DEFAULT_EDGE = 2
T_SOFT_LIMIT = 20        # was 10 — let positions ride longer

# crazy1 EMERALDS constants (only AGGRO thresholds changed)
...
E_IMB_THRESH = 0.12
E_AGGRO_POS = 20         # was 30 — CLEAR earlier
E_AGGRO_TARG = 5          # was 15 — CLEAR harder
...
class Trader:
    ...
    # ══════════════════════════════════════════════════════════════
    # EMERALDS — crazy1 with MORE aggressive CLEAR (target 1,200+)
    # ══════════════════════════════════════════════════════════════
    ...
    def trade_emeralds(self, od, pos):
        ...
        # AGGRESSIVE CLEAR — pos>20→5 (was pos>30→15)
    ...
    # ══════════════════════════════════════════════════════════════
    # TOMATOES — v10 SUBMITTED (penny-jump) + higher limits
    # ══════════════════════════════════════════════════════════════
